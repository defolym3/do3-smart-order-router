...
import { Protocol } from '@defolym3/do3-router-sdk';
import { ChainId, Currency, CurrencyAmount, Token, TradeType } from '@defolym3/do3-sdk-core';
import { FeeAmount, Pool } from '@defolym3/do3-v3-sdk2';
import { Pair } from '@defolym3/do3-v2-sdk/dist/entities';
...
export async function calculateGasUsed(
  ...
  //   )[1];
  // } else if ([ChainId.OPTIMISM, ChainId.BASE, ChainId.SHAPE_TESTNET].includes(chainId)) {
  ...
  if ([ChainId.OPTIMISM, ChainId.BASE, ChainId.SHAPE_TESTNET, ChainId.MODE].includes(chainId)) {
    l2toL1FeeInWei = calculateOptimismToL1FeeFromCalldata(
      route.methodParameters!.calldata,
      l2GasData as OptimismGasData
    )[1];
